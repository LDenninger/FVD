...
"""
class FVDCalculation2:

    def __init__(self,
                  method: str = 'videogpt',
                  frame_sample_strategy: str = 'middle',
                  batch_size: int = 16,
                  ):
        self.method = method
    ...
    def __call__(self, real_path_list: Union[List[str], List[Path]], generated_path_list: Union[List[str], List[Path]], model_path = "model"):
        """
        ...
        # 
        chunked_real_paths = [real_path_list[i:i+self.batch_size] for i in range(0, len(real_path_list), self.batch_size)]
        chunked_generated_paths = [generated_path_list[i:i+self.batch_size] for i in range(0, len(generated_path_list), self.batch_size)]

        features_real, features_generated = [], []
        progress_bar = tqdm(enumerate(zip(chunked_real_paths, chunked_generated_paths)), desc="Computing logits", total=len(chunked_generated_paths))
        for i, (real_paths, generated_paths) in progress_bar:
            real_videos = [load_video(p, sample=self.frame_sample_strategy, num_frames=None) for p in real_paths]
            generated_videos = [load_video(p, sample=self.frame_sample_strategy, num_frames=None) for p in generated_paths]
            real_videos = torch.stack(real_videos, dim=0)
            generated_videos = torch.stack(generated_videos, dim=0)

            features_real_ = self._get_logits(model, real_videos, device)
            features_generated_ = self._get_logits(model, generated_videos, device)

            features_real.append(features_real_)
            features_generated.append(features_generated_)

        features_real = torch.cat(features_real, dim=0)
        features_generated = torch.cat(features_generated, dim=0)

        fvd = self._frechet_distance(features_real, features_generated)

        return fvd.detach().cpu().numpy()
    @torch.no_grad()
    def _get_logits(self, i3d_model, videos, device):
        logits = []
        batch = self._preprocess(videos, self.target_resolution, method=self.method)
